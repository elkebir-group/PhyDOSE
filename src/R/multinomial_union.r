...
calc_cells <- function(df_path, sample, tree_num, u_list, fn_rate, gamma){
  
  df_fam <- df_family(df_path)
...
num_cells <- function(dfFamily, u, fn, gamma){
  ...
  if(length(dfFamily) == 0){
    ...
    cells <- invert.pmultinom(lower = lower_bound, probs = prob_vec, target.prob = gamma, method="exact")
  }else{
...
calc_prob <- function(prob_vec, cells){
  ...
  pmult <- pmultinom(lower = lower_bounds, size = cells, probs = prob_vec, method = "exact")
