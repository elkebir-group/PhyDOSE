...
public:
  ...
  /// Return set of clones
  const std::set<StringSet> getCloneSet() const
  {
    if (_cloneSet.empty())
    {
      for (NodeIt v(_tree); v != lemon::INVALID; ++v)
      {
        _cloneSet.insert(_nodeToMutations[v]);
      }
    }
    return _cloneSet;
  }
  ...
  const StringSet& nodeToMutations(const Node u) const
  {
    return _nodeToMutations[u];
  }
