  ...
  , _transEdgeSet(other._transEdgeSet)
{
  if (other._idToNode.size() > 0)
  {
    lemon::digraphCopy(other._tree, _tree)
      ...
      .nodeMap(other._nodeToMutations, _nodeToMutations)
      .run();
  ...
  , _transEdgeSet()
{
  ...
  {
    // make nodes
    for (const std::string& nodeLabel : nodes)
    {
      Node v = _tree.addNode();
      _nodeToId[v] = nodeLabel;
      _idToNode[nodeLabel] = v;
    }
    
    for (const std::string& nodeLabel1 : nodes)
    {
      Node v1 = other.getNodeByLabel(nodeLabel1);
      
      std::vector<std::pair<int, std::string>> levels;
      for (const std::string& nodeLabel2 : nodes)
      {
        if (nodeLabel1 == nodeLabel2) continue;

        Node v2 = other.getNodeByLabel(nodeLabel2);
        if (isAncestor(v2, v1))
        {
          levels.push_back(std::make_pair(other.level(v2), nodeLabel2));
        }
      }
      ...
      if (levels.empty())
      {
        ...
        _root = _idToNode[nodeLabel1];
      }
      else
      {
        std::sort(levels.begin(), levels.end());
        _tree.addArc(_idToNode[levels.back().second], _idToNode[nodeLabel1]);
      }
    }
...
{
  _nodeToMutations[u].clear();
  _nodeToMutations[u].insert(_nodeToId[u]);
  if (u != _root)
  {
    _nodeToMutations[u].insert(_nodeToMutations[parent(u)].begin(),
                               _nodeToMutations[parent(u)].end());
  }
