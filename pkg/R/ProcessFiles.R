...
create_tree <- function(lines, vecs, key){
    #vecs <- rev(lines[start:end])
    
    treeMat <-  matrix(rep(0, nrow(key)**2), 
                       nrow=nrow(key), 
                       ncol=nrow(key), 
                       dimnames=list(clones=as.character(0:(nrow(key)-1)),
                                     mutations=key$character_index)
                       )
...
##########################################################
#
#     requires rgraphviz/graph package
#     
#
#
###########################################################
...
get_matrices_and_trees <- function(charVec, start=1){
    ...
    for(i in 1:length(charVec)){
        if(str_detect(charVec[i], pattern = "edges") &&  !start_tree){
            s <- i + 1
            start_tree <- TRUE
            next
        }
        if((str_detect(charVec[i], pattern = "edges") || 
        ...
        if(str_detect(charVec[i], "#samples")){
            charVec[i] <- str_replace(charVec[i],"#", "")
            header <- str_split(charVec[i], " ")
            num_samples <- as.numeric(header[[1]][1])
            ct <- 0
            next
        }
        if(str_detect(charVec[i], "#characters")){
            charVec[i] <- str_replace(charVec[i],"#", "")
            header <- str_split(charVec[i], " ")
            num_muts <- as.numeric(header[[1]][1])
            next
        }
        ...
        if(ct == num_samples*num_muts && fmat){
            # finished fmatrix
            num_f <- num_f + 1
            key <- filter(fmatrix, sample_index == 0) %>% 
